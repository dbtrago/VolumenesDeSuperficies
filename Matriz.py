...
def calcularV(funcion, a, b, c, d, n, m, delta_x, delta_y):
    ...
    for i in range(int(n)): #for(int i=w, i==b,i++)
        fila = []  # Inicializar una fila vacía en cada iteración
        fila_suma = []

        #print("2 w: " + str(w), "v: " + str(v))
        for j in range(int(m)):
            valor = (round(w,1), round(v,1))
            fila.append(valor)
            suma = suma + (delta_a * evaluarFuncion(funcion, w, v))
            fila_suma.append("{:05.1f}".format(round(suma,3)))
            w = w + delta_x
    ...
    print("\nMatriz con los puntos medios (W,V): ")
    for fila in matriz:
        print(fila)

    # Imprimir la matriz con las sumas resultante
    print("\nMatriz con las sumas resultantes: ")
    for fila2 in matriz_suma:
        print(fila2)
    return suma
